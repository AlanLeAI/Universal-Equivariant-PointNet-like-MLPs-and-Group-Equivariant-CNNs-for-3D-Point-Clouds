...
from model.group_pointconv import GroupPointConvDensityClsSsg as GPointConvClsSsg
import provider
import numpy as np
...
def parse_args():
    ...
    parser.add_argument('--r_group', type=int, default=4)
...
def main(args):
    ...
    print("Model rotation with {} R-Group !".format(args.r_group))

    '''CREATE DIR'''
    experiment_dir = Path('./experiment_rotation/')
    ...
    '''MODEL LOADING'''
    num_class = 40
    classifier = GPointConvClsSsg(num_class, args.r_group)
    ...
    for epoch in range(start_epoch,args.epoch):
        ...
        for batch_id, data in tqdm(enumerate(trainDataLoader, 0), total=len(trainDataLoader), smoothing=0.9):
            ...
            jittered_data = provider.shift_point_cloud(jittered_data, shift_range=0.5)
        ...
        # test_random_angel(classifier, testDataLoader, coordinates="Rx", rotation_angle=np.pi / 4)

        if acc >= best_tst_accuracy:
            best_tst_accuracy = acc
            PATH = str(file_dir)+ "/best_model_epooch_{}_acc_{}.pt".format(global_epoch,best_tst_accuracy)
